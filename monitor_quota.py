...
def fetch_usage(auth: dict, opener: urllib.request.OpenerDirector, auth_path: Path, timeout: int, debug: dict | None = None, retries: int = DEFAULT_RETRY_LIMIT) -> dict:
    debug = debug if debug is not None else {}
    token = refresh_access_token(auth, opener, auth_path, timeout, retries)
    headers = {
